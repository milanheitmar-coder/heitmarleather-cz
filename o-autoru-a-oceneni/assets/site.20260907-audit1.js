...
function hlTrackConfirmedLeadOnce() {
  ...
  if (details.lead_type === 'diy_box_order' && typeof window.fbq === 'function') {
    // Diagnostická vlastní událost: pokud by Meta standardní Lead filtrovala,
    // tato událost ukáže, že potvrzení objednávky se na stránce opravdu spustilo.
    window.fbq('trackCustom', 'DIYOrderConfirmed', {
      value: 2490,
      currency: 'CZK'
    });
  }
...
function bindLeadForms(){
  document.querySelectorAll('form[data-contact-form]').forEach(function(form){
    ...
    form.addEventListener('submit', function(e){
      const isDiyOrder = form.getAttribute('name') === 'objednavka-diy-sada';
      const leadType = isDiyOrder ? 'diy_box_order' : 'commission_enquiry';

      if (isDiyOrder) {
        // U DIY objednávky pouze synchronně uložíme potvrzovací příznak do sessionStorage
        // a necháme formulář odeslat nativně přes Netlify. Meta Lead se odešle až na
        // potvrzovací stránce /odeslano/, kde už nehrozí, že ho přeruší navigace.
        window.hlPrepareConfirmedLead({
          source: 'netlify_form',
          form_id: form.id || '',
          lead_type: leadType,
          content_name: 'Objednávka DIY sady kožené kabelky',
          value: 2490,
          currency: 'CZK'
        });
        return;
      ...
      // Ostatní poptávkové formuláře využijí stejné potvrzení na děkovací stránce.
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  settingsLinks.forEach(function(link){
    link.addEventListener('click', function(e){
      ...
      showBanner();
      if (banner) {
        try { banner.scrollIntoView({behavior: 'smooth', block: 'end'}); } catch (err) {}
      }
    });
  });
