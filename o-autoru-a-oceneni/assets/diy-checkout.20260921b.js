(function(){
  ...
  let lastTrackedColor = '';

  function quantityValue(){
    return Math.max(1, Math.min(3, parseInt(qty.value || '1', 10)));
  }

  function trackColorSelection(selectedColor){
    if (!selectedColor || selectedColor === lastTrackedColor) return;
    lastTrackedColor = selectedColor;
    const count = quantityValue();
    try {
      if (typeof window.gtag === 'function') {
        window.gtag('event', 'select_color', {
          item_id: 'diy-crossbody-cz',
      ...
      if (typeof window.fbq === 'function') {
        window.fbq('trackCustom', 'SelectColor', {
          content_ids: ['diy-crossbody-cz'],
          content_name: 'DIY Box Crossbody',
      ...
      console.warn('Color tracking failed', err);
    }
  }
  ...
  function trackBeginCheckout(){
    ...
    try {
      if (sessionStorage.getItem('hl_diy_begin_checkout_v1') === '1') {
    ...
    beginCheckoutTracked = true;
    try { sessionStorage.setItem('hl_diy_begin_checkout_v1', '1'); } catch (e) {}

    const count = quantityValue();
    const checkoutValue = total();
    try {
      if (typeof window.gtag === 'function') {
        window.gtag('event', 'begin_checkout', {
          ...
          items: [{
            item_id: 'diy-crossbody-cz',
            item_name: 'DIY Box Crossbody',
            item_variant: color.value || undefined,
            price: PRICE,
            quantity: count
          }]
        });
  ...
  color.addEventListener('change', function(){
    trackColorSelection(color.value);
    updateSummary();
  });
  ...
  form.addEventListener('focusin', function(event){
    ...
    if (target.matches('input[name="name"], input[name="email"], input[name="phone"], input[name="street"], input[name="city"], input[name="postal_code"], textarea[name="message"]')) {
      trackBeginCheckout();
    }
  });
